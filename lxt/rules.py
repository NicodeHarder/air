...
class epsilon_lrp_fn(Function):
    ...
    *inputs: at least one torch.Tensor
    ...
    def forward(ctx, fn, epsilon, *inputs):
        ...
        # Extra modification (Adapted from Leander Weber's modification):
        # Add requires_grad to the parameters of the function
        # in order to later compute the weight relevances
        ...
        # Extra Modification:
        # Store the module itslef to
        # later access the weight parameters
        ctx.fn = fn
    ...
    def backward(ctx, *out_relevance):
        ...
        weight = ctx.fn.weight
        # compute gradient of weight with respect to output

        # print(f"Before, GPU memory: {torch.cuda.memory_allocated('cuda') / 1e9:.2f}GB allocated")

        weight_grad = torch.autograd.grad(
            outputs, weight, relevance_norm, retain_graph=True
        )

#         print(f"After, GPU memory: {torch.cuda.memory_allocated('cuda') / 1e9:.2f}GB allocated")

        # Formula of weight relevance according
        # to ECQx paper, equation 5
        # https://iphome.hhi.de/samek/pdf/BecArXiv21.pdf
        weight_relevance = weight_grad[0] * weight
        # Storing the weight relevance
        weight.relevance = weight_relevance

        # print(weight_relevance.sum())
